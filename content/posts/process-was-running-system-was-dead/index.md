...
The uncomfortable part is that this was supposed to be watched. The container was set up to be monitored by Datadog, and the errors were all there - the application was writing every one of those hundred-odd out-of-memory events into the log file we ship to Datadog. They just never made the trip. The upload had been broken for over a month, an unrelated problem nobody had noticed, so the errors piled up in a file on the box, and no alert ever fired. You can't alert on data that never arrives. The availability checks, meanwhile, were working perfectly - and they were only ever asking whether the service was up, which it was.
